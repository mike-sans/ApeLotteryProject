...
// This script is just the original VRFV2Wrapper.sol with one small modification where you input the link-eth price
// as the first argument in place of the address of a priceFeed aggregator being the second argument. One also needs 
// to have a VRFCoordinatorV2Mock.sol to do local testing of the Chainlink VRF V2's direct funding method.
    ...
    VRFV2WrapperInterface
{
    ...
    function onTokenTransfer(
        ...
        int256 weiPerUnitLink = _getFeedData();
        uint256 price = _calculateRequestPrice(
            callbackGasLimit,
            tx.gasprice,
            weiPerUnitLink
        );
    ...
    function _getFeedData() private view returns (int256) {
        ...
        // (, weiPerUnitLinkOrig, , timestamp, ) = LINK_ETH_FEED.latestRoundData();
        weiPerUnitLink = LINK_ETH_PRICE;
